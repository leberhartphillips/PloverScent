...
knitr::opts_chunk$set(tidy = TRUE,cache=TRUE,warning = FALSE,
                      comment = "#>",fig.width = 6,fig.height = 4)
```

## Intro

This is R Markdown document provides all code necessary code to run the analysis on chemical profiles of preen gland samples of three Madagascan plover species (Kittlitz´s-, Madagascar- and White-fronted). Samples have been processed by gas-chromatography and peaks were called using **Xcalibur 2.0.5**. All further steps are reproducible with the code delivered within this document.

```{r,fig.align='center',echo=FALSE}
knitr::include_graphics("docs/plover-pics.png")
```
...
Since one MP (FH47040) was sampled twice during brooding, we average the to samples, thereby we avoid pseudoreplication. 
...
## Now we can start to analyse pattern using  Nonmetric multidimensional scaling (NMDS)
...
nmds_plotter(nmds = nmds_m1,main = "Adults") # plot 
```` 
```{r}
beta1 <- vegan::betadisper(vegan::vegdist(scent_m1,method = "bray"), factors_m1$Species) # Dispersion effect
anova(beta1)
TukeyHSD(beta1) # post-hoc test
vegan::adonis(scent_m1~factors_m1$Species*factors_m1$Brood_Status,strata = factors_m1$Nest_Brood, permutations = 999,method = "bray")
```